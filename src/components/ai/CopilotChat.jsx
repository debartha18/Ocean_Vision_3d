    ...
    <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-3.5 text-[#CCD0CF] text-xs">
      {messages.length === 0 && (
        <div className="flex flex-col items-center justify-center my-auto py-8 text-center px-4">
          <div className="w-12 h-12 rounded-xl bg-[#0D202B] border border-[#193544] flex items-center justify-center text-[#0C969C] mb-3">
            <Bot className="w-6 h-6" />
          </div>
          <h4 className="text-sm font-bold text-[#CCD0CF] mb-1">Nerida · AI Ocean Copilot</h4>
          ...
          <div className="text-[10px] text-[#6BA3BE] font-mono bg-[#0A1720] px-3 py-1.5 rounded-xl border border-[#193544]">
      ...
      {isLoading && (
        <div className="flex items-start gap-2 max-w-[85%] self-start animate-in fade-in duration-200">
          <div className="w-6 h-6 rounded-lg bg-[#0D202B] border border-[#193544] flex items-center justify-center text-[#0C969C] shrink-0 mt-0.5">
            <Bot className="w-3.5 h-3.5" />
          </div>
          <div className="p-3 rounded-2xl rounded-tl-none bg-[#0D202B] border border-[#193544] text-[#CCD0CF] flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#0C969C] animate-pulse"></span>
            <span className="w-1.5 h-1.5 rounded-full bg-[#168FA0] animate-pulse delay-100"></span>
            <span className="w-1.5 h-1.5 rounded-full bg-[#274D60] animate-pulse delay-200"></span>
            <span className="text-[10px] text-[#8FA8B2] font-mono ml-1">Nerida is thinking...</span>
          </div>
        </div>
      )}
  ...
  if (isUser) {
    return (
      <div className="flex items-start gap-2 max-w-[85%] self-end flex-row-reverse">
        <div className="w-6 h-6 rounded-lg bg-[#142D3A] border border-[#214555] flex items-center justify-center text-[#0C969C] shrink-0 mt-0.5">
  ...
  return (
    <div className="flex items-start gap-2 max-w-[95%] self-start flex-col">
      <div className="flex items-start gap-2 w-full">
        ...
        <div className="flex-1 p-3.5 rounded-2xl rounded-tl-none bg-[#0D202B] border border-[#193544] text-[#CCD0CF] shadow-sm relative group">
          {/* Top Bar: Provenance Badges + Copy Button */}
          <div className="flex items-center justify-between gap-2 mb-2.5 pb-2 border-b border-[#193544]">
          ...
          {toolResults && toolResults.length > 0 && (
            <div className="mt-3 pt-2.5 border-t border-[#193544] flex flex-col gap-1.5">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[10px] font-mono border ${
                      isSuccess
                        ? 'bg-[#0A1720] border-[#0C969C]/40 text-[#CCD0CF]'
                        : 'bg-[#0A1720] border-amber-500/40 text-amber-300'
                    }`}
                  >
                    <CheckCircle className="w-3 h-3 text-[#0C969C] shrink-0" />
                    <span className="font-semibold text-[#0C969C]">Nerida ✓</span>
          ...
          {dataPointsUsed && dataPointsUsed.length > 0 && (
            <div className="mt-2.5 pt-2 border-t border-[#193544]">
              <button
                onClick={() => setShowWhy(!showWhy)}
                className="flex items-center justify-between w-full text-[10px] text-[#8FA8B2] hover:text-[#CCD0CF] font-mono py-0.5 cursor-pointer"
              >
                <span className="flex items-center gap-1">
                  <Info className="w-3 h-3 text-[#0C969C]" />
              ...
              {showWhy && (
                <div className="mt-1.5 p-2 rounded-lg bg-[#06141B] border border-[#193544] text-[9px] font-mono text-[#8FA8B2] flex flex-col gap-1">
      ...
      {suggestions && suggestions.length > 0 && (
        <div className="ml-8 mt-1 flex flex-wrap gap-1.5 items-center">
          <span className="text-[10px] text-[#637C87] font-mono flex items-center gap-1 mr-1">
            <Sparkles className="w-2.5 h-2.5 text-[#0C969C]" /> Suggested:
          </span>
          {suggestions.map((sug, sIdx) => (
            <button
              key={sIdx}
              onClick={() => onSuggestionClick && onSuggestionClick(sug)}
              className="text-[10px] font-mono bg-[#142D3A] hover:bg-[#183746] text-[#8FA8B2] hover:text-[#CCD0CF] border border-[#214555] hover:border-[#0C969C]/50 rounded-full px-2.5 py-1 transition-all cursor-pointer shadow-sm active:scale-95"
            >
              {sug}
            </button>
          ))}
