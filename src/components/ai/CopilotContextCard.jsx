...
export default function CopilotContextCard({ oceanContext }) {
  ...
  return (
    <div className="mx-3 mt-2 rounded-xl border border-[#193544] bg-[#0D202B] overflow-hidden select-none transition-all">
      <div 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="flex items-center justify-between px-3 py-2 bg-[#0A1720] cursor-pointer hover:bg-[#142D3A] transition-colors"
      >
        <div className="flex items-center gap-1.5 text-[11px] font-bold text-[#0C969C] uppercase tracking-wider">
      ...
      {!isCollapsed && (
        <div className="p-2.5 grid grid-cols-2 gap-2 text-[10px] font-mono border-t border-[#193544]">
          <div className="flex items-center gap-1.5 text-[#CCD0CF]">
            <Compass className="w-3 h-3 text-[#0C969C] shrink-0" />
            <div className="truncate">
              <span className="text-[#637C87]">Basin: </span>
              <span className="font-bold text-[#CCD0CF]">{basin}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 text-[#CCD0CF]">
            <Waves className="w-3 h-3 text-[#6BA3BE] shrink-0" />
            <div className="truncate">
              <span className="text-[#637C87]">Depth / Wave: </span>
              <span className="font-bold text-[#6BA3BE]">{depth}m / {wave}m</span>
            </div>
          </div>
